...
def test_cache_prevents_saving_if_price_unchanged(scraper, mock_db, mock_cache):
    scraper.cache.is_price_unchanged = MagicMock(return_value=True)
    product = Product(title="Product", price=100.0, image_url="http://example.com/image.jpg")
    scraper._save_product_if_valid(product)
    scraper.db.save_product.assert_not_called()
